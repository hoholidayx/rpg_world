...
Play API 独占进程内事件 Hub：`POST /play-api/v1/internal/events` 使用 `RPG_WORLD_PLAY_EVENT_TOKEN` Bearer 鉴权，`GET /play-api/v1/events/stream` 为 WebUI 提供全局 SSE。每订阅者有界队列满时丢最旧事件；不实现 SQL outbox、补发或消费确认。首版只支持 Play API 单进程/单 worker。WebUI 根 `Providers` 只挂一个 EventSource bridge，严格解析并把最近 50 条事件留在 Zustand 内存；当前不得据此 Toast、跳转、自动刷新 Query 或写 localStorage。
...
- `PlayEventBridge` 只允许在根 `Providers` 挂载一次；当前 store 仅用于打通接收链路，不得触发 Dream/Derivation 业务 UI 或持久化。
