...
│   ├── status/                   # 状态表薄适配（rpg_data SQL 索引 + CSV 内容源）
│   ├── summary/                  # 对话摘要
...
| [N+3] Status Tables | system | 普通状态表，不包含 `当前场景.csv` | ★★★★ 高频变化 |
...
`rpg_data` 状态表 service 用 `builtin_key="scene"` 表达这一类特殊状态，且仍由 story
...
`rpg_data.services.status.StatusTableService` 是状态表管理入口：SQL 是完整索引，
...
业务索引。默认清理不在 SQL 索引中的 workspace/story/session 目录和未索引 status CSV；
关闭开关是 `RPG_WORLD_BOOTSTRAP_DELETE_ORPHAN_DIRS=false`。
...
- **Status**: SQLite 索引 + CSV 内容源。SQL 保存 type/template/story mount/session copy 和 workspace-relative `relative_path`；CSV 使用 UTF-8 BOM（`utf-8-sig`），Excel 兼容
