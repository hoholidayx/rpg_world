...
│   │   ├── agent_types.py        #     protocol/telemetry/mailbox 类型兼容导出
│   │   ├── loop.py               #     turn runner 兼容导出
│   │   └── derivation_service.py #     derivation runtime 兼容导出
│   ├── scene/                    # 场景状态（时间/地点/属性）
...
│   ├── summary/                  # 对话摘要
│   ├── main_llm.py               # 主模型选择兼容导出入口
│   ├── turns.py                  # Agent turn 类型兼容导出入口
...
| `rpg_context.py` | 历史导入兼容入口，只重导出 `models.py` 的同一批类 |
...
生产代码使用 canonical 类型模块；`agent/agent_types.py`、`agent/loop.py`、根级 `main_llm.py` 和 `turns.py` 只保留兼容导出，不承载实现。
