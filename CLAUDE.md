...
│   │   ├── turn/                 #     request/snapshot/runtime/preparation/orchestrator
...
`RPGGameAgent` 是公开门面和 session 生命周期 owner，不再分别维护同步/流式两套业务编排。`rpg_core/agent/turn/` 的职责是：

```text
TurnRequest                            调用方原始、不可变输入
  → TurnPreprocessor
  → TurnSnapshotResolver
      → TurnExecutionSnapshot         mode/style/policy 快照
  → TurnOrchestrator                  同步/流式共享模板
      → TurnExecutionPlan             snapshot + 主 LLM/RP Module 选择快照
      → Context window gate
      ...
      → sync/stream runner
      → commit → AgentReply/SSE DONE → post-commit side effects
```
...
- `TurnPreparation` 是正式 turn 的 Context、turn-local tools 与 schemas 统一构建入口；context preview 复用 snapshot resolver 和共享 Context builder 的只读路径，但不创建 transaction。
...
- `StatusSubAgent` 负责可选预裁定：需要 outcome 的同批状态预写全部延后，结果以 `outcomeCode / label / narrativeGuidance / reason / actor?` 在主 Agent 首次调用前进入 `RP_MODULES` runtime section。漏判或预裁定失败时主 Agent 仍可补判；已预裁定时主 Agent schema 隐藏 outcome 工具，内部重复调用仍幂等复用。
...
agent.send(user_input)
  → QueueItem(TurnRequest) 串行出队
  → TurnPreprocessor：CommandDispatcher + 玩家角色校验（旁路不入历史）
  → TurnSnapshotResolver：mode/style policy
  → TurnOrchestrator 解析 MainLLMSelection + RPModuleSelectionSnapshot
  → 当前主 Agent Context 窗口门禁使用同一组不可变快照（不计本次 input；拒绝时不创建 scratch）
  → TurnRuntime 创建 AgentTurnTransaction / TurnScratch / RPModuleTurnRuntime
  → StatusSubAgent.update() 按 policy 先判断外部实质变数
  ...
  → TurnPreparation → builder.build() + turn-local tools/schemas
  ...
  → summary compression / story memory extraction 作为 commit 后副作用
```
