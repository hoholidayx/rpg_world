...
`channels/telegram/session_flow.py` 消费；callback 只携带 `TelegramActionRegistry` 生成的短 token，
并在执行前校验 TTL、chat 与当前 session。`/session_create [title...]` 在 Telegram 渠道内直接调用
...
| 停止 | Telegram 当前不提供主动打断入口；内部 request ID 只用于 stream 关联和进程关闭时的安全清理 |
