...
# CLI / Telegram 的 turn、命令和 Session mutation 通过 agent_client 访问 Agent 服务
...
├── run_telegram.py                # Telegram 入口（AgentClient + Session Reference 只读组装）
...
├── memory_retrieval/             # 业务无关检索基础包
│   ├── planning/                 #   RetrievalQuery / QueryPlan / rule planner
│   ├── retrieval/                #   sqlvec / keyword / raw md / hybrid
│   ├── rerank/                   #   pointwise rerank
│   └── storage/                  #   SQLite repository / vector / text index
├── rpg_memory/                   # RP 记忆领域与应用编排
│   ├── recall/                   #   RP query/policy、在线召回 manager 与调试 CLI
│   ├── story/                    #   Story Memory store/application
│   ├── persistent/               #   Persistent Memory ledger/projection
│   ├── dream/                    #   Shallow/Deep、Proposal/Apply/恢复
│   └── benchmark/                #   RP 固定召回 benchmark
...
│       ├── characters.py         #   Story 直属角色卡 APIs
│       ├── lorebook.py           #   Story 直属世界书 APIs
│       ├── status_tables.py      #   Story 定义、Session 运行表 APIs
...
只有 `run_agent.py` 进程持有 `AgentManager` 和 `RPGGameAgent`。Agent 进程使用 `rpg_memory` 做 RP 召回与 Context 投影，底层检索委托业务无关的 `memory_retrieval`；`run_dream.py` 使用无框架的 `rpg_memory.dream` 做离线提炼并经 `rpg_data` 写 Persistent Memory。`rpg_core` 不运行 Dream consolidation、不写该账本，二者也都不得持有 Provider。
...
Dream service 是 Session 级离线记忆提炼边界，只持有 `rpg_memory.dream`、`rpg_data` 与 loop-owned `llm_client`。它通过 `dream.shallow` / `dream.deep` biz key 获取远端 Provider facade，不导入 Agent runtime、`MemorySubAgent` 或 `llm_service`。Play API 只用 `DreamClient` 代理手动管理请求；Dream 中断不得影响 Agent Context 构建、聊天或其它服务。v1 不提供入站 Bearer 鉴权，配置层强制监听 localhost/loopback IP；非 loopback host 必须启动失败，不支持非本地暴露。

Dream Proposal 状态机、恢复、Apply、Persistent Memory 生命周期与 Context projection 的唯一业务 owner 是 `rpg_memory.dream`；Story Memory 的 normalize、exact dedupe、merge、Evidence、version 和 Context projection 统一归 `rpg_memory.story.application`。`rpg_data` 只通过 `dream_memory` / `story_memory` 提供 frozen DTO、CRUD、CAS、批量读写与无业务语义事务，不得恢复状态迁移、动作分支、active-limit、来源指纹、Evidence 有效性或 Story Memory 合并策略；业务调用方不得直接访问 `gateway.database` 或 Peewee record。

`memory_retrieval` 只拥有 chunk、FTS、vector、通用 query plan、Hybrid 融合、rerank、索引协调和存储，不得导入 `rpg_memory`、`rpg_core` 或 `rpg_data`。RP 的上下文查询适配、`overall.md` 排除、记忆粒度偏好、传闻/尝试/承诺 rerank prompt 和远端能力编排由 `rpg_memory.recall` 注入；依赖方向固定为 `rpg_memory → memory_retrieval`。
...
│   └── rpg_memory（RP 召回与 Persistent Memory 只读投影）
│       └── memory_retrieval（索引、FTS/vector、融合与 rerank）
...
`MemoryRecallManager.create()` 只创建本地壳，`initialize()` 只建立 text index、keyword、raw-md 与 watcher coordinator，二者都不访问 LLM Service。首次 `recall()` / `reindex()` 才并发懒解析 embedding、query planner 和 reranker；远端能力失败时继续使用 keyword/raw-md/rule-based fallback，未就绪能力在之后调用中重试。每个 MemoryRecallManager 的操作锁只约束该 session，因此同 session 不会并发操作同一 SQLite/索引状态，不同 session 仍可并行。watchdog 所在线程不得执行索引、embedding 或 SQLite。
...
`rpg_data` 的 catalog 关系仍是 workspace 下有多个 story、story 下有多个 session。Character、Lorebook 与 Status 内容资产由 Story 直接拥有，不存在 Workspace 资产库或 Story mount；同名资源可以分别存在于不同 Story，但所有 CRUD 都必须同时校验 `workspace_id + story_id`，资源不能跨 Story 复用。

Story 主数据字段中，`summary` 是短摘要，`story_prompt` 是 Story 专属固定系统提示词，会通过 fixed layer 参与上下文渲染。会话开局真源是 `rpg_story_openings` 中按顺序保存的 0–3 条稳定 Opening（标题＋正文），Story 表不再保存旧 `first_message` 列。Opening 正文与 Story Prompt 当前只支持白名单变量 `{USER_PLAY_ROLE_NAME}`；数据库和 API 返回原始模板，选中 Opening 在首次绑定且历史为空时渲染后持久化，Story Prompt 在 turn snapshot 中按 session 角色渲染。未知单花括号变量在保存边界失败，不执行 Jinja、表达式或递归替换。第一条 Opening 是默认项；CLI/Telegram 与未指定 Opening 的调用方使用它。

状态表的 SQLite `document_json` 是 Story 定义和 Session 运行表的正文真源。`rpg_story_status_tables` 直接归属 Story，可选绑定同一 Story 的一个 Character；`rpg_session_status_tables` 只包含 `story_copy | session_native`，Story 副本通过可空的 `source_story_status_table_id` 记录来源。`status_kind` 只允许 `scene | normal`。新 Session 的复制策略由 `rpg_core.session.SessionCatalogService` / `SessionStatusLifecycleService` 决定，`rpg_data.StatusDataService` 只复制调用方明确给出的 Story 状态表 ID，或原子应用调用方准备好的 typed reset plan。Story 定义后续修改或删除不覆盖既有 Session 副本；删除来源时副本保留且来源 ID 由外键置空。`rpg_data` bootstrap 仅为缺失运行表的 Session 执行 materialize，不承载新的 Session 创建策略，也不硬编码 demo 数据。Bootstrap 默认保留 SQL 未索引目录，只有显式设置 `RPG_WORLD_BOOTSTRAP_DELETE_ORPHAN_DIRS=true` 才允许清理并记录结果。
...
`当前场景` 是 Story 直接拥有的 `status_kind="scene"` 特殊状态表，展示名可以自定义；Session 创建时与其它 Story 状态表一起复制。多张 scene 表存在时，v1 消费排序第一张 active scene。LLM 结构写权限由 `agent.scene.allow_runtime_key_changes` 控制且默认关闭：现有字段继续完整注入并允许更新 value，但不能新增、删除或重命名 key；只有显式开启后才恢复非锁定 key 的运行时增删能力。该开关只收紧 Agent 工具，不改变 Play API / `rpg_data` 的手工管理 CRUD。
...
玩家扮演角色是 session 级运行语义。绑定状态对外只暴露 `bound | invalid`：缺失绑定、角色不存在于当前 Story、snapshot 损坏或 snapshot 的角色/Story 身份与当前归属不一致，都统一视为 `invalid`。WebUI 不在进入 SessionRoom 前拦截，而是在 SessionRoom 内打开不可取消的角色选择弹窗；没有可选角色时显示阻塞空态。CLI / Telegram / Agent API 允许创建空 session，但普通消息在绑定前只返回固定编号角色列表，不调用 LLM、不写 user history。
...
| 能力 | 当前实现 |
|---|---|
| 启动方式 | `uv run python -m run_telegram`（Agent 写链路走 `agent_client`，已提交资料走本地 Session Reference） |
...
SQL History 与派生 Summary 两组只读查询工具由 `agent.lookup_tools.enabled` 统一控制，默认关闭。关闭时主 Agent 的 base registry 不注册 `history_search` / `history_read` / `summary_search` / `summary_read`，Outcome、状态 Route/Update 和 soft Plot 也不获得 LookupToolSet，裁定共享前缀不得宣称存在这些能力；开启后四种 mode 与上述各裁定阶段统一恢复。History 工具定位并读取当前 Session 已提交的 user/assistant 主消息，是核对原话、主体和精确结果的历史真源；Summary 工具搜索并读取 `summaries/*.md` 的批次/总览归纳，用解析后的白名单 front matter 和 `resolvedTurnRange` 快速定位旧剧情。Summary 正文是次级证据，不被动注入裁定 Context；无冲突且语义明确时可辅助裁定，与当前 Scene/状态、较新 SQL History 或 Evidence-valid Memory 冲突时服从更高真源，存在歧义、缺失来源范围或需要精确事实时必须回查 History。Summary 的 batch turn 范围优先使用 SQL `summary_batch_id` 聚合，缺失时才回退文件 front matter；overall 按 `last_batch_id` 聚合覆盖的 batch。

开启 Lookup 工具时，Outcome、状态 Route、每个状态 Update 与每个 soft Plot 候选复用组合后的只读 LookupToolSet，但各阶段分别拥有一个 `agent.adjudication.max_lookup_tool_rounds` 正整数预算；History 与 Summary 调用共同消耗该预算，不是两份额度。查询 transcript 与 `reasoning_content` 只在当前阶段临时回传，不持久化；查询与终结工具混用时不得执行终结工具，预算耗尽后只额外允许一次终结调用。`list_files / read_file / write_file / grep` 只保留源码、显式注入能力和直接测试，不再由 `AgentToolService` 默认注册；显式注入的 `WriteFileTool` 在 OOC 仍按既有策略隐藏。四个查询工具的 arguments、excerpt 和正文都属于敏感内容，`verbose_logging` 只能记录工具名与 `<redacted chars=N>`，不得输出原文。
...
- `TurnExecutionSnapshot` / `TurnExecutionPlan` 在 Context 门禁前形成，生成期间不可变；Story/Persistent Memory 投影与渲染后的共享裁定 Context 也进入 plan，门禁、主 Context、Status 与 soft Plot 共用同一份 turn 快照。新增 turn 配置应先进入 snapshot，再由 policy/preparation 消费。
...
`HybridRetriever` 负责把 `sqlvec / keyword / raw_md` 三路结果合并，`MemoryRecallManager.hybrid_search()`
...
另一方面让纯场景数据 snapshot 随 user message 进入历史，便于后续摘要和记忆按时间顺序归纳；
...
`runtimeKeyLocked` 只保护 key 不被删除或重命名，不限制 value 更新。Scene 的非空 `updateRule` 仅由
`SceneTracker.get_context()` 注入本轮运行时 Context，持久化 user history 使用的 snapshot 不包含这些规则。

普通 `STATUS_TABLES` 层只展示 session 运行时表 ID、表名、作为“用途与更新规则”的
`description`、完整 KV、`runtimeKeyLocked` 和逐字段 `updateRule`，不展示模板来源或通用作用范围；只有本轮实际注册对应工具时才宣称可写，OOC/无工具 Context 明确只读。绑定角色的普通表进入独立的
“角色状态表”段落并按 `characterName` 分组；当前角色绑定不触发额外工具或业务行为。
角色绑定入库必须校验角色 name 非空。Story 副本 metadata 缺角色名时，优先按复制时保存的
`characterId` 读取当前 Story Character；来源定义仍存在时也可从 `source_story_status_table_id`
对应的角色关联修复，成功后回填副本 metadata。仍无法解析的表记录 warning 并从 LLM 上下文排除，
不得合并到“未知角色”分组。
...
- Story 可同时挂载多条线性大纲和多个事件池。大纲节点引用稳定 Story 事件并保存固定 `SceneTime`；通过确定性资格规则的池按正整数 `selectionWeight` 和稳定 Session/turn seed 加权抽取，不提供有限轮次保底。`random` 池再按事件 `selectionWeight` 抽主候选；soft 主候选按池 `candidateBatchSize`（默认 3、范围 1–5）加权无放回补齐仅含 soft 事件的 batch，并只调用一次 Judge 选出至多一个最适宜事件，事件权重只表示 batch 召回概率。forced 主候选直接注入；`sequential` 池仍严格按 position 推进并忽略事件权重与 batch，大纲 priority 保持原义。自动 selector 不按每个 `neutral | ic | gm` turn 运行：只有上一个成功提交 turn 的 active Scene document 最终发生实际变化，才留下供下一次非 OOC turn 消费的一次机会。消费 turn 在 `StatusPreflight` 后使用最新 scratch Scene，最多选择一个到期大纲节点和一个池事件；当前 turn 若又改变 Scene，则原子留下供再下一轮使用的新机会。OOC、命令、模块禁用、失败或取消不消费也不创建机会；OOC 仍可在沙盘工具边界内检查定义、标记或清空手动快照。被任意大纲节点引用的事件始终排除自动 pool lane，不看大纲/节点启用或 Session 覆盖；删除全部引用后才回池，手动标记仍可绕过。
...
- 每个 Session 最多一条待注入快照。快照保存来源 Story/event/pool 标识与版本、冻结标题/directive 和事件 metadata；来源事件或池后续编辑、移动或删除都不改变快照，也不阻止注入。下一次成功的 `neutral | ic | gm` turn 在自动调度前把它作为 forced pool lane 注入，即使没有 Scene 调度机会或 Scene 时间缺失也必须执行；显式手动注入忽略源定义的启用、时间窗、大纲绑定、重复与全部冷却规则，无 SceneTime 时也可解除目标事件已有的事件级冷却锚点，但不启动、刷新或清除池级冷却。它占用本轮 pool lane，并抑制同事件的大纲重复。手动决策写 `selection_origin="manual"`，允许 `scene_time`/ordinal 为 null；自动决策仍必须有 SceneTime。
- `forced` 主候选到时直接暂存为 triggered，不获得 Judge Context 或工具；单个大纲/sequential soft 候选和 random soft batch 都通过 `agent.plot_scheduler` 独立 biz key 调用 LLM。random batch 的终止工具必须返回 batch 内 `selectedEventId`、`suitable` 与 `reason`；最终只对选中事件注入或落一条 deferred，未选候选不产生 retry、事件冷却或池冷却状态。非法选择与 Judge 错误只在加权主候选上记录 error。Judge 读取模块中立的共享裁定前缀（裁定权限、Story Prompt、世界书、玩家绑定/角色卡、Persistent Memory、Story Memory）、当前 scratch scene/普通状态表、最近 N 个完整原始可推进世界 turn 和当前输入；不被动读取 Summary/Recall，不读叙事/格式指令、Message Mode 或任何 RP Module 提示词。只有 `agent.lookup_tools.enabled=true` 时，关键事实不确定的候选才可在独立预算内按需使用 `summary_search` / `summary_read` 或 `history_search` / `history_read`。Judge `reason` 由 schema 与 parser 双重限长，避免无界元数据突破主 Context 门禁预留。
...
- 大纲节点不重复；池事件可配置基于世界内分钟的事件级重复冷却。池 lane identity 固定为 `event_id`，事件移到其它池后仍沿用已触发、延期和事件级冷却状态。池的 `cooldownMinutes` 默认为 `0`，按当前 SceneTime 与该池最新一条已提交 scheduler-origin pool `triggered` 决策计算共享冷却；`container_id` 既记录当时所属池，也是池冷却锚点，无需新增 Session 状态。未到期时整池跳过并可回退到下一可用池；manual、outline、`deferred`、`error` 均不影响锚点。大纲 lane 独立调度，但结构绑定事件不再进入自动 pool lane。
...
- Play WebUI 只在 `/plot-scheduling` 独立页管理定义、覆盖和运行态；运行态不轮询、不调用 Judge。Story 卡展示池冷却配置和事件大纲绑定，Session 卡展示池剩余冷却、跳过原因、锚点，以及可展开的完整决策与 `eventSnapshot`。决策历史按 `id DESC` + `beforeId` 分页，不能改为 `turn_id` 游标，否则同 turn 的 outline/pool 两条记录可能漏页。内置 catalog 包含 Plot Scheduler；默认模块只在 Story 创建时挂载，不追溯修改既有 Story。
...
| characters | `play_api/routers/characters.py` | Story 直属角色卡与角色详情 CRUD |
| lorebook | `play_api/routers/lorebook.py` | Story 直属世界书条目 CRUD |
| status-tables | `play_api/routers/status_tables.py` | Story 直属状态表定义与 Session 运行表 CRUD |
...
Story Memory 已由 `rpg_session_story_memories` 持久化，Persistent Memory 则由 Dream SQL 账本持久化；两者都不属于上述文件 Manager 模式，也都不维护 JSON 文件或 watcher。每个 turn plan 和 Context Preview 前分别通过 `asyncio.to_thread()` 读取 `rpg_data` 的 Evidence-valid 投影并返回不可变快照，worker 线程关闭自己的 Peewee connection；turn 必须把两份快照放入 `TurnExecutionPlan`，门禁、实际主 Context 与共享裁定 Context 显式共用，Preview 捕获自己的快照，任一读取失败时对应 store 使用上一份只读快照。
...
`session_id` 与数据访问引用，通过 `rpg_data` service 实时读取当前 Session 所属 Story 的直属资源，
不读取旧 JSON/目录结构，也不注册 `FileWatcher`。Character/Lorebook 是否进入上下文由 Story 归属决定；
Status 使用 Session 创建或 reset 时形成的独立运行副本。

`rpg_data.services.status.StatusDataService` 是状态表的公开类型化持久化入口：SQL row 内的
`document_json` 是正文真源；service 不通过目录扫描发现状态表。它负责 Story 定义、Session
document、角色关联 read model、last-write-wins 诊断与调用方指定的原子 document
batch，不决定 Scene、Context 或运行时写入策略。`StatusTableAdministrationService` 负责 Story/Session 表管理规则，
`SceneStatusService` 负责 Scene 字段约束与 active Scene，`StatusContextService` 负责角色名修复和 Context
可见性，`StatusManager` 负责 Agent 当前 turn 即时更新与 bootstrap 写入资格。composition root 从
`get_data_service_gateway().status` 取得 Data Service 后显式注入，不新增 per-service 全局 getter，也不让
业务服务持有整个 Gateway。通用存储写操作仍支持 header 名称、行匹配和 key/value selector；LLM 在已有
normal Session 表内通过 `status_table_set_values` 更新 value，并通过 `status_table_edit_fields` 原子新增、
改名或删除字段，但不能 CRUD 整张表。key/value 写入以 `StatusTableDocument` 的逻辑 key/value 为准，
不依赖 UI 列标题。
状态 document 固定为 `schemaVersion=2`；每个 `StatusTableRow` 只允许 `key`、`value`、
`runtimeKeyLocked`、`updateRule` 和 `metadata`。旧版本、旧频率/周期字段与其它未知属性直接拒绝。
所有字段 value 都可由 Agent 在当前 turn 即时更新；空 `updateRule` 使用通用事实变化条件，非空规则
只是额外语义指导。normal 表的 `description` 若允许开放字段，还应定义动态 key 的领域、命名/value 格式及
创建、改名、删除条件。`runtimeKeyLocked=true` 只禁止该字段删除或改名，不限制 value 更新或同表新增。
`/clear` 删除既有 `story_copy` 并按当前 Story
定义重建；`session_native` 表保留 ID 和完整结构，但所有 value 置空。同名原生表与当前 Story 定义
冲突时 reset 原子失败。
...
- 状态表管理页只保留 `Story 定义` 与 `Session 运行时` 两个视图：前者维护当前 Story 直属定义与角色绑定，后者维护指定 Session 的 Story 副本和 Session 原生表。删除 Story 定义不删除既有 Session 副本。
...
- **Character/Lorebook**: SQLite Story 直属资源；所有读取和写入同时校验 workspace/story 归属
- **Status**: SQLite document 真源。SQL 保存 Story 定义、Session copy/origin/source/status_kind/sort_order 与封装后的 `document_json`；对外通过 `StatusTableDocument` / `StatusTableRow` dataclass 访问
