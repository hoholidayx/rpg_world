...
character/lorebook 是例外：`rpg_core.character.CharacterManager` 和
`rpg_core.lorebook.LorebookManager` 只保存 `session_id`，通过 `rpg_data`
service 实时读取当前 session 绑定 story 的角色卡/世界书挂载，不再读取 JSON，
也不注册 `FileWatcher`。是否进入上下文由 story 挂载关系决定。
