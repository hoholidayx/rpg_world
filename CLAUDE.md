...
| [N+4] RP Modules | system | RP 模块动态运行态；Dice / text_output_format 默认均为空 | ★★★★ 动态 |
...
RP Modules 不是通用 skill 体系。骰子、战斗、物品、关系、输出格式等能力必须定义为围绕 RP 语义的模块：
...
- prompt-only 输出格式也作为 RP Module 接入 fixed layer；`text_output_format` 默认启用，用 `<rp-narration>` 和 `<rp-character name="...">` 约束 assistant 正文中的旁白/角色分离。
- 动态运行态只在模块确有临时状态时进入 `RP_MODULES` system layer；Dice MVP 与 `text_output_format` 的 `get_runtime_sections()` 返回空列表。
