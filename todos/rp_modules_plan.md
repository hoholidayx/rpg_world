...
Dice 的主要体验不是让玩家手动输入 `/roll`，而是在普通沉浸式 RP 中，由 Agent 在关键的不确定节点调用工具。`/roll` 和 `/check` 是兜底、调试和玩家明确想手掷时的入口；正常玩法仍是自然语言对话。
...
### `/check <expr> dc=<n> [reason...]`

示例：

```text
/check 1d20+2 dc=12 潜行越过甲板
```
...
- `/roll`、`/check` 被命令分发器拦截，不进入 LLM，不写 history。

### API / WebUI 契约测试

- `GET /play-api/v1/sessions/{session_id}/commands` 能返回 `/roll`、`/check`、`/rp_modules`。
...
8. 注册 `/rp_modules`、`/rp_module`、`/roll`、`/check` 命令。
...
- `/roll`、`/check` 可用，输出兼容 Play WebUI、CLI、Telegram。
