...
- Session、Memory、Status 与 Media 的 canonical 存储契约分别位于 `rpg_data.model.session`、`rpg_data.model.memory`、`rpg_data.model.status` 和 `rpg_data.model.media`。
- `rpg_data.models` 暂时兼容重导出已迁移类型；新代码优先从 canonical 模块导入，不再向巨型兼容模块增加新的跨域类型。
- TTS 等尚未整改类型只有在对应业务域发生真实整改时再迁移，不为目录整齐进行批量搬运。
