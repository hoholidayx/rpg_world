...
TurnPlanResolver
  ...
  ├─ PersistentMemory / StoryMemory immutable snapshots
  └─ AdjudicationContextSnapshot
       │
       ▼
TurnRuntimeFactory
  ├─ 主 Context 窗口门禁（不计本次 input；为最多两条 Plot 指令和有界元数据预留）
  ├─ 解析本轮主 provider
...
| `TurnExecutionSnapshot` / `TurnExecutionPlan` | frozen | 玩家角色、Story Prompt、mode/style policy、主模型、RP Module 与 Plot Schedule 选择 | Context 门禁前解析，本轮不再变化 |
...
Outcome、Route、每个隔离 Update 与每个软 Plot 候选都从同一个不可变 `AdjudicationContextSnapshot` 开始。它使用显式白名单，只包含：

- 裁定事实与权限边界；
- 本 turn 已渲染的 Story Prompt；
- 世界书；
- 本 turn 固化的玩家角色绑定和角色卡；
- Evidence 仍有效的 Persistent Memory；
- Evidence 仍有效的 Story Memory。

它明确不复用主 Fixed Layer，也不读取 RP Module registry/runtime；因此 Summary Layer/正文、Recall、叙事风格、正文格式、核心叙事/状态同步协议、Message Mode，以及当前或未来任何 `rp_module:*` 提示词都不能被动进入这些裁定。`agent.lookup_tools.enabled=false` 时，共享权限 section 不提及 Summary/History 查询能力；开启后才说明 Summary 的次级证据地位及四个查询工具的使用边界，实际 Summary 内容仍只能由当前阶段按需查询。每个阶段只在共享前缀后追加自己的窄契约、当前 scene/状态目标、近期原始历史窗口和当前输入。RP Module 快照仍决定 Outcome/Plot 能力是否存在及其业务配置，但模块提示词本身不参与判断。
...
| StatusSubAgent | 共享裁定 system messages → 阶段 system contract → 动态 user；查询后追加临时 assistant/tool transcript | Outcome、Route、scene Update 与各单表 Update 分开 |
| MemorySubAgent | 一条 pipeline system contract → 一条动态 user | Recall、Story、Summary、Batch Summary、Overall Summary 分开 |

StatusSubAgent 不使用主 Context 的 system 合并逻辑，而是复用独立的裁定前缀；MemorySubAgent 仍保持自己的 pipeline system + 动态 user 结构。主 Context 是否合并不会直接改变 SubAgent 请求，低命中更常由不同阶段 schema、动态 user 内容、服务端缓存策略或共享缓存容量造成。
...
### 可选的 History 与 Summary 查询工具

`agent.lookup_tools.enabled` 默认是 `false`，主 Agent 与所有裁定阶段均不注册或获得下列工具。显式设为 `true` 后，主 Agent、Outcome、状态 Route、每个状态 Update 和每个软 Plot 候选统一获得两组互补的只读工具：
...
两组工具都不把临时 transcript 持久化回消息表。开关开启时，裁定阶段各自使用独立的有界循环，预算不跨阶段共享，但同一阶段的 History/Summary 共用一个预算。旧文件工具仍保留实现和直接测试，但不属于默认 registry。
