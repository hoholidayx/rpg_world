<!-- run-id:20260717T060156Z-2e8f8f0b -->
# RP Memory 召回基准总结

- Run ID：`20260717T060156Z-2e8f8f0b`
- 开始时间（UTC）：`2026-07-17T06:01:56.037039Z`
- Git revision：`abccb93f0b714a01bd280788a49ad9830358aebd`（dirty：`true`）
- 执行命令：`uv run python -m rp_memory.benchmark suite --record --offline-only`
- 本地完整报告：`/Users/hoholiday/Projects/Pycharm/rpg_world/data/benchmarks/results/rp-memory-recall-full-20260717T060156Z-2e8f8f0b.md`
- 门禁状态：仅记录，不阻断变更或发布。
...
| 完整报告 | `/Users/hoholiday/Projects/Pycharm/rpg_world/data/benchmarks/results/rp-memory-recall-full-20260717T060156Z-2e8f8f0b.md` |
...
服务探测：`skipped_disabled` — configured LLM service intentionally not contacted

| 能力 | 状态 | Provider | Backend / 模型 | Dimension | 原因 |
|---|---|---|---|---:|---|
| embedding | `skipped_disabled` | `-` | `-` / `-` | - | --offline-only disables configured-provider probes and execution |
| planner | `skipped_disabled` | `-` | `-` / `-` | - | --offline-only disables configured-provider probes and execution |
| reranker | `skipped_disabled` | `-` | `-` / `-` | - | --offline-only disables configured-provider probes and execution |
...
- 向量召回：`skipped_disabled`，本次未包含该路径；原因：--offline-only disables configured-provider probes and execution
- Planner 扩展查询：`skipped_disabled`，本次未包含该路径；原因：--offline-only disables configured-provider probes and execution
- rerank：`skipped_disabled`，本次未包含该路径；原因：--offline-only disables configured-provider probes and execution
