<!-- run-id:20260718T132425Z-0a0f3d2c -->
# RP Memory 召回基准总结

- Run ID：`20260718T132425Z-0a0f3d2c`
- 开始时间（UTC）：`2026-07-18T13:24:25.994474Z`
- Git revision：`d034d996176797f665320acf58d3f376deeebe2a`（dirty：`false`）
- 执行命令：`uv run python -m rp_memory.benchmark suite --record`
- 本地完整报告：`/Users/hoholiday/Projects/Pycharm/rpg_world/data/benchmarks/results/rp-memory-recall-full-20260718T132425Z-0a0f3d2c.md`
- 门禁状态：仅记录，不阻断变更或发布。
...
| 完整报告 | `/Users/hoholiday/Projects/Pycharm/rpg_world/data/benchmarks/results/rp-memory-recall-full-20260718T132425Z-0a0f3d2c.md` |
...
|---|---|---|---|---:|---|
| embedding | `skipped_probe_failed` | `qwen_embedding_0_6b` | `llama` / `Qwen3-Embedding-0.6B-Q8_0.gguf` | - | llama-cpp-python is not installed |
| planner | `skipped_probe_failed` | `memory_query_planner` | `llama` / `Qwen3-0.6B-Q8_0.gguf` | - | llama-cpp-python is not installed |
| reranker | `skipped_probe_failed` | `memory_rerank` | `llama` / `qwen3-reranker-0.6b-q8_0.gguf` | - | llama max_parallel_models exceeded: configured=2 requested=('rerank', 'data/models/qwen3-reranker-0.6b-q8_0.gguf', 4096, 0) |
...
| `configured.embedding.qwen_embedding_0_6b` | `skipped_probe_failed` | - | - | - | - | - | - | - | - | - | - | - | - | - |
| `configured.planner.memory_query_planner` | `skipped_probe_failed` | - | - | - | - | - | - | - | - | - | - | - | - | - |
| `configured.rerank.memory_rerank` | `skipped_probe_failed` | - | - | - | - | - | - | - | - | - | - | - | - | - |
...
- 本次没有成功执行 embedding 检索路径；`offline.keyword_rule` 是不包含 embedding、LLM Planner 或 rerank 的纯离线关键词基线。它适合稳定回归候选覆盖，但无法判断旧状态与当前状态、承诺与完成、尝试与结果等事实阶段；因此该路径的 Forbidden 指标只用于诊断词法候选污染，不能代表完整模型配置路径的质量。
...
- 向量召回：`skipped_probe_failed`，本次未包含该路径；原因：llama-cpp-python is not installed
- Planner 扩展查询：`skipped_probe_failed`，本次未包含该路径；原因：llama-cpp-python is not installed
- rerank：`skipped_probe_failed`，本次未包含该路径；原因：llama max_parallel_models exceeded: configured=2 requested=('rerank', 'data/models/qwen3-reranker-0.6b-q8_0.gguf', 4096, 0)
