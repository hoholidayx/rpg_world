...
设计原则：WebUI 负责体验上限，Telegram 负责触达效率；二者必须共享同一套 workspace/session 语义，避免同一个故事在不同渠道分裂。Dashboard 与 Play 两个 Web 项目共享 FastAPI、`rpg_core`、`rp_memory` 后端能力，不在前端复制角色、世界书、状态或 RP 规则。
...
`AgentManager`、`RPGGameAgent`、`rp_memory` 和 llama lazy worker。Dashboard API、
Play API、CLI、Telegram 只通过 `AgentClient` 调用 Agent 服务。
...
| `dashboard_api/settings.yaml` | Dashboard API 监听参数、API 日志、聊天日志开关 |
...
dashboard_api/settings.local.yaml
...
Dashboard / Play API 的监听和日志分别放在 `dashboard_api/settings.yaml`、`play_api/settings.yaml`。
...
- `dashboard_api/settings.yaml` — Dashboard API 监听与日志
