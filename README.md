...
- [架构](#架构)
  - [进程隔离架构](#进程隔离架构)
  - [`rpg_data` 数据访问架构（独立文档）](docs/rpg-data-architecture.md)
...
- **2026-07-20：Status / Scene 业务与数据层拆分。** 状态模板/挂载/Session 表管理、Scene 字段规则与 active Scene、角色名修复与 Context 可见性、运行时/bootstrap 写入资格统一归 `rpg_core`；`StatusDataService` 只保留 typed CRUD、关联 read model、last-write-wins 诊断和原子 document batch。Status 存储契约迁入 `rpg_data.model.status`。
...
- **2026-07-13：Agent Turn 与状态更新固定编排。** Narrative Outcome 与状态更新拆分为独立阶段；状态更新先 Route，再按 scene 和单张普通表隔离执行。状态目标采用 best-effort：单个目标失败只回退该目标，其他成功目标保留且主流程继续；整个 turn 仍只在主 runner 成功后统一提交。scene 继续以 `status_kind="scene"` 作为数据真源，但在主 Context 和更新工具上走专用路径。字段契约现已由 2026-07-24 的 Schema v2 硬切取代。完整设计、时序和失败语义见 [Agent Turn 与状态更新编排](docs/agent-turn-orchestration.md)。
...
- Character 与 Lorebook 由 Story 直接拥有，不存在 Workspace 级资产库或 Story mount；不同 Story 可以各自保存同名资源，CRUD 始终校验 `workspace_id + story_id`。角色一级叙事字段只有 `name + description`；description 只写身份、经历和客观事实，性格、说话方式、行为倾向与心理使用二级内置 kind 标签，且演绎详情自动附带 `scope:npc_portrayal`。
...
- 玩家身份在 Context 门禁前固化进 `TurnExecutionSnapshot`，由门禁、主 Agent、StatusSubAgent 与 Context Preview 共用。fixed layer 的 `[player_character]` 标签块是身份唯一真源；角色卡只在 Context 投影时标注为 `PLAYER_CHARACTER` 或 `NPC`，不依赖角色 metadata。玩家卡从 Fixed Layer 排除 `scope:npc_portrayal` 详情，NPC 卡保留；只有当前 GM turn 托管时才在后置动态层注入玩家演绎详情。当前绑定覆盖冲突的旧历史、摘要和记忆，但不会自动改写这些既有数据。
...
- canonical 存储契约位于 `rpg_data.model.status`，`rpg_data.models` 只保留兼容重导出。`StatusDataService` 负责 Story 定义、Session document、角色/来源关联 read model 和原子批量写入；`StatusTableAdministrationService`、`SceneStatusService`、`StatusContextService` 与 `StatusManager` 分别承接管理后台用例、Scene、Context 和 Agent 运行时策略。
...
- 所有已有字段的 value 均可由 Agent 在当前 turn 即时更新。`neutral | ic | gm` 还可在已有 normal Session 表内通过 `status_table_edit_fields` 原子创建、改名或删除字段；读取继续来自每轮完整状态 Context，不提供整表 CRUD，OOC 与命令只读。新字段追加到表尾并使用 `runtimeKeyLocked=false / updateRule="" / metadata={}`，改名原位保留原 value 与作者策略；锁定字段只禁止自身改名/删除，不妨碍 value 更新或同表新增其他字段。value 是字符串，可按表约定表达数值、枚举、列表、简短描述或当前事实状态。表 `description` 集中提供整表共同语义、value 格式和即时更新规则；开放字段的 normal 表还应说明动态 key 领域、命名/value 格式与创建、改名、删除条件。row `updateRule` 只补充字段专属条件，不预设数值模型，空值使用通用“事实已明确且值实际变化”条件。两者都不产生定时调度或数据库级写入门禁。状态表保存需要每轮可见和更新的当前状态；Memory 更适合按时间累积的叙事历史，但当前事实、承诺、联系或事件状态仍可成为字段。完整编排见 [Agent Turn 与状态更新编排](docs/agent-turn-orchestration.md#状态字段契约与即时更新)。
...
- 运行时直接新建的会话表写入 `rpg_session_status_tables`，`origin="session_native"`；状态系统不再维护逐字段进度或回复后的慢更新任务。
- `/clear` 删除旧 `story_copy` 并按当前 Story 定义重建；`session_native` 表保留 ID、结构、metadata 和 `updateRule`，仅将所有 value 置空。同名原生表与当前 Story 定义冲突时整个 reset 回滚。
...
- `当前场景` 是 Story 直属 `status_kind="scene"` 定义复制出的特殊 Session 表；多张 scene 表存在时消费排序第一张。它使用同一 v2 行契约，非空 `updateRule` 只作为本轮运行时 Context 指导，不写入历史 scene 快照。LLM 默认只能修改已有 key 的 value；`agent.scene.allow_runtime_key_changes=true` 才允许通过 scene 工具增删非锁定 key，`runtimeKeyLocked` 只在结构编辑时保护 key，管理端手工 CRUD 不受影响。
...
`MediaApplicationService` 是来源选择、图库规则、删除门禁、背景状态机和 worker 恢复的统一业务入口；worker 不直接访问数据层。`MediaDataService` 负责 Job/Blob/Asset/Gallery/Background/Evaluation 的 typed persistence、复杂 read model、CAS claim、引用查询和原子 completion，canonical DTO 位于 `rpg_data.model.media`。
...
`AgentTurnTransaction` 是“内存 scratch + 短 commit 点”，不是跨 LLM 调用的长数据库事务。user/assistant message、Narrative Outcome、Plot Scheduler 判断、scene/status 都先暂存；即时状态阶段按 scene/单表目标独立 checkpoint，目标失败只恢复该目标并继续后续目标及主 runner。主 runner 完整成功后才统一提交仍保留的 scratch，流式 DONE 只在 commit 成功后发送；主 turn 取消或失败仍丢弃全部 scratch。story-memory 与 summary 属于 commit 后任务，不回滚已提交 turn。
...
- `rpg_core/rp_modules/` 是 RP 业务模块体系，不做通用 skill 体系。`message_mode` 提供代码内置的 `neutral | ic | ooc | gm` 本轮语义；`narrative_outcome` 负责主 Agent 的剧情分支随机裁定；`plot_scheduler` 按 Scene 时间选择大纲节点与事件池事件；`dice` 只保留表达式解析和手动调试命令；`text_output_format` 仍由 fixed layer contributor 约束 assistant 正文使用 RP XML 标签。
...
- `RP_MODULES` 是模块动态运行态层，位置在 `STATUS_TABLES` 后、`USER_MESSAGE` 前。非 neutral 的 `message_mode` 指令和 GM 托管详情在这里按 turn 注入；Narrative Outcome 平时依靠 fixed contract 判断隐式变数，检测到明确随机意图时加入本轮强制裁定指令。若 StatusSubAgent 已预裁定，本轮不再注入 Narrative Outcome fixed section，只在动态层注入最终结果和明确的状态工具边界。
...
prefix cache 以 provider 最终序列化/tokenized 请求的共同前缀为准，不以结构化层、消息边界或整条 system message 的应用层 hash 为独立缓存单元。动态 system 位于 Hot History 后，变化不会截断更早的稳定指令和历史前缀；四种 message mode 共用字节相同的 Fixed Layer，模式指令与 GM 托管详情只改变后置 `RP_MODULES`。rolling history 自身发生滑窗时仍会改变共同前缀。整条 system/context hash 不同仍可能命中更早的部分 token，实际缓存效果只看 provider usage 返回的 hit/miss token。开启 `verbose_logging` 后，`TurnPreparation` 会在最终主 Agent messages 和 tool schemas 完成后、首次主 LLM 调用前输出一次不含正文的 `contextHash` / `systemHash` / `toolsHash`、逐消息 `index/role/hash/chars`、role 计数和工具名；后续工具 round 不重复输出这条初始指纹。

`当前场景` 在数据层是 Story 直属 `status_kind="scene"` 定义复制出的 Session SQL document，在主 Context 中则是高优先级 `[scene]` user prefix，不进入普通 `STATUS_TABLES`。Status Route 只在本轮涉及且存在可用 scene 工具时选择它；scene 只走专用工具，不存在回复后更新路径。默认关闭结构编辑时，已有字段都保持可见且可改 value，`scene_attr` 只接受现有 key，`scene_time` 只在已有 `时间` key 时出现，`scene_del_attr` 不注册；空 scene 完全不暴露写工具。完整差异见 [scene 的特殊语义](docs/agent-turn-orchestration.md#scene-的特殊语义)。
...
- `message_mode` 是无配置的可选模块，mode/prompt 不持久化到 Workspace。neutral 始终可用且无额外提示；模块有效时 WebUI 才展示 IC/OOC/GM，模块无效却请求这些模式时在 LLM/history 前返回 `message_mode_unavailable`。OOC 不写世界事实，neutral/IC/GM 可推进世界。
- 高频或临时模块状态才进入 `RP_MODULES` 动态层；`message_mode` 注入非 neutral 指令，Narrative Outcome 为明确随机意图加入本轮强制指令，并把 StatusSubAgent 已暂存的裁定结果注入主 Agent 首次调用。
...
- Plot Scheduler 有效时，OOC 与 GM 主 Agent 可用 `plot_sandbox_read` 读取当前沙盘及池冷却/大纲绑定诊断，用 `plot_event_mark_next(event_id, title?, directive?)` 标记下一次非 OOC turn；`event_id=null` 直接清空。临时 title/directive 只冻结到一次性快照，不修改原事件；省略时冻结原内容。快照不随来源事件编辑或删除改变，显式手动注入不依赖 Scene 调度机会，并忽略启用、时间窗、大纲绑定、重复与全部冷却规则；它可清除目标事件级冷却锚点，但不启动、刷新或清除池级冷却，OOC 本轮本身不会消费或推进世界。
...
Play WebUI 左侧“剧情调度”页面提供三类大面板：大纲时间线、事件池定义、会话运行态。定义卡展示池冷却、事件数、自动池候选数及大纲专用数；运行态读取 Scene 时间、禁用覆盖、池剩余冷却/跳过原因/锚点和已提交决策，可展开查看完整决策字段与原始 `eventSnapshot`，但不触发 LLM、不轮询，也不向 SessionRoom 增加 HUD。决策历史以 `id DESC` + `beforeId` 稳定分页，单页最多 200 条；同一 turn 的 outline/pool 两条记录不会因分页丢失。管理接口集中在：
...
Outcome 判定与状态更新由代码拆成独立阶段：预裁定成功后停止 Route 并将结果注入主 Agent；无需裁定时才执行状态目标路由与隔离即时更新。Outcome 的失败补判语义不受单目标 best-effort 策略影响。完整正常链路和失败路径见 [StatusSubAgent 固定编排](docs/agent-turn-orchestration.md#statussubagent-固定编排)。
...
每个自动剧情 turn 最多产生一条裁定；同一 turn 重复工具调用复用第一次结果。权重在 turn 开始时形成快照，生成中修改只影响下一 turn。裁定与 user/assistant message、即时状态变更在同一个短数据库事务中提交；取消、provider 失败或 commit 失败都不留记录。
...
`memory_retrieval` 不导入 `rpg_memory`、`rpg_core` 或 `rpg_data`。`overall.md` 排除、RP 记忆粒度评分、最近可推进世界 turn/玩家/场景查询扩展，以及传闻/尝试/承诺的 pointwise rerank prompt 都留在 `rpg_memory.recall`，通过窄 query/policy/prompt 接口注入通用 planner、retriever 和 reranker。
...
- **Deep**：以当前主消息表的 neutral/IC/GM user/assistant 历史为事实真源；Full 可全局校准，Incremental 只处理新增、修改、删除及 Evidence 受影响的记忆。
    ...
    telegram:
      bots:
        main:
          ...
          # 已提交回复后显示 Outcome 裁定与 triggered Plot 注入气泡；默认开启。
...
- [`docs/rpg-data-architecture.md`](docs/rpg-data-architecture.md) — `rpg_data`、Data Service、Gateway、Repository、事务与类型所有权的正式规范
- [`docs/agent-turn-orchestration.md`](docs/agent-turn-orchestration.md) — Agent turn、Outcome、状态即时路由与事务完整编排
