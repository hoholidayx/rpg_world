...
# 启动所有模块（读取 settings.yaml，按配置启动 Dashboard API / Telegram / CLI）
uv run python -m run
...
`MODULES=dashboard_api`、`MODULES=telegram`、`MODULES=dashboard_api,telegram` 临时覆盖启用模块。
...
cfg.dashboard_api_enabled       # modules.dashboard_api.enabled
