...
- SessionRoom 提供宽屏图像工作室：从 1–20 个连续已提交 turn 生成并编辑 `VisualBrief`，异步提交图片任务，查看 Session Gallery、来源陈旧标记，设置/清除背景以及删除非背景资产。
- SessionRoom 已提交的 assistant 回复提供按需 TTS：首次点击创建持久异步任务，服务端单音色生成分段 MP3；页面内使用全局单播放器连续播放，不写 message metadata 或 localStorage。
...
`PATCH /play-api/v1/sessions/{sessionId}/player-character`，由 Play API 转发角色 ID 与可选 `storyOpeningId` 到 Agent service，并复用 `/role_bind <角色序号> [开局序号]` 的后端绑定逻辑；首次空会话通过只读 Opening options 接口完成“角色 → 开局”向导后一次提交。前端不要直接推断场景在场角色或首个角色作为“你”的身份。
...
完整本地链路通常需要分别启动 Agent service、Media service、Play API 和 WebUI。前端仍只访问 `/play-api/v1`，不会直连默认 `http://127.0.0.1:8011/media/v1` 的 Media service。

首版默认使用 `rpg_media/settings.yaml` 中的 `local_file` Demo Provider。把 PNG/JPEG/WebP 样图放入仓库根目录的 `data/media_provider/`（或用同级 profile 覆盖 `providers.local_file.source_dir`）后，该 Provider 才会显示为可用；目录缺失或无合法图片时，简报仍可生成和编辑，但提交按钮保持禁用。真实产物由后端校验魔数后写入 `{workspace_root}/assets/images/{sha256}.<ext>`。
