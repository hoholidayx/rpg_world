...
export function SessionDerivationDialog({
  ...
  return (
    <ModalPortal>
      ...
      >
        <div
          className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/30 px-4 py-8 backdrop-blur-sm dark:bg-slate-950/65"
          onMouseDown={(event) => {
        ...
        >
          <section
            ...
            tabIndex={-1}
            className="relative max-h-[calc(100dvh-4rem)] w-full max-w-xl overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/70 outline-none dark:border-slate-700 dark:bg-slate-950 dark:shadow-black/50"
          >
            <header className="flex items-center justify-between border-b border-slate-200 px-6 py-5 dark:border-slate-800">
              ...
              <button
                ...
                onClick={onClose}
                disabled={pending}
                className="flex h-11 w-11 items-center justify-center rounded-lg text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-40 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-slate-50 sm:h-9 sm:w-9"
                aria-label="关闭"
              >
                <X size={18} />
              </button>
            ...
            >
              <div className="space-y-5 px-6 py-5">
                ...
                {error ? (
                  <p role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold leading-6 text-rose-700 [overflow-wrap:anywhere] dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200">
                    创建失败：{error}
                  </p>
                ) : null}
              </div>
              ...
              <footer className="flex items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 px-6 py-4 dark:border-slate-800 dark:bg-slate-900">
                ...
                <button
                  type="submit"
                  disabled={pending}
                  className="inline-flex h-11 items-center gap-2 rounded-lg bg-violet-600 px-4 text-sm font-black text-white shadow-lg shadow-violet-100 transition hover:bg-violet-700 disabled:cursor-not-allowed disabled:opacity-60 dark:shadow-violet-950/40 sm:h-10"
                >
