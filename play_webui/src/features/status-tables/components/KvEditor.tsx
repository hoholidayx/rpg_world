    ...
    <div>
      ...
      <div className="overflow-x-auto rounded-b-xl border-x border-b border-slate-200">
        <div className="min-w-[860px]">
          <div className="grid grid-cols-[170px_minmax(180px,1fr)_minmax(260px,1.2fr)_110px_60px] items-center gap-3 border-b border-slate-100 bg-slate-50 px-3 py-3 text-xs font-extrabold text-slate-600">
            <span>Key</span>
            <span>Value</span>
            <span>更新规则</span>
            <span>运行时锁定 key</span>
          ...
          {draft.rows.length ? draft.rows.map((row, index) => (
            <div key={index} className="grid grid-cols-[170px_minmax(180px,1fr)_minmax(260px,1.2fr)_110px_60px] items-center gap-3 border-b border-slate-100 bg-white px-3 py-3 last:border-b-0">
              <input
                value={row.key}
      ...
      <p className="mt-2 text-xs leading-5 text-slate-400">
        所有字段都由 Agent 在当前 turn 即时判断更新；规则为空时仅在事实明确且值实际变化时更新。运行时锁定只保护 key 不被删除或重命名，不限制 value 更新。
      </p>
    </div>
  )
}
