...
function outlineReferenceCounts(schedule: PlotSchedule) {
  const counts = new Map<number, number>()
  schedule.outlines.forEach((outline) => {
    outline.nodes.forEach((node) => {
      counts.set(node.eventId, (counts.get(node.eventId) ?? 0) + 1)
    })
  })
  return counts
}
            ...
            <>
              ...
              {target.kind === 'pool' ? (
                <>
                  ...
                  <p className="-mt-3 text-sm font-semibold leading-6 text-slate-500">池内任意事件被自动调度成功注入后，整个池按 SceneTime 进入冷却。手动标记与大纲注入不会改变该锚点。</p>
                  <Toggle checked={enabled} label="启用此事件池" onChange={setEnabled} />
                </>
              ) : null}
            </>
          ) : null}
    ...
    <section className={`${panelClass} mb-6 overflow-hidden`}>
      <div className="flex flex-col gap-4 bg-gradient-to-r from-violet-600 to-indigo-600 px-6 py-5 text-white xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-start gap-4">
          ...
          <div><p className="text-lg font-black">RP Module · 剧情动态调度</p><p className="mt-1 max-w-3xl text-sm font-semibold leading-6 text-violet-100">定义属于 Story；只有挂载并启用模块后，已提交的 Scene 净变化才会为下一次非 OOC turn 留下自动调度机会。</p></div>
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      {!schedule.pools.length ? <Empty>暂无事件池。先创建一个池，再添加可复用的剧情事件。</Empty> : schedule.pools.map((pool) => {
        ...
        const poolLaneCount = events.length - outlineBoundCount
        return (
...
function RuntimeView({ schedule, sessions, sessionId, onSessionChange, runtime, sessionModule, busy, onEventOverride, onNodeOverride, onEarlier, onLatest, viewingOlder }: {
  ...
  runtime: Awaited<ReturnType<typeof getSessionPlotSchedule>> | undefined
  sessionModule: RPModuleConfig | undefined
  busy: boolean
  onEventOverride: (eventId: number, disabled: boolean) => void
  onNodeOverride: (nodeId: number, disabled: boolean) => void
  onEarlier: (beforeId: number) => void
  onLatest: () => void
  ...
  const eventById = new Map(schedule.events.map((event) => [event.id, event]))
  const poolById = new Map(schedule.pools.map((pool) => [pool.id, pool]))
  const bindingByEventId = new Map(
    (runtime?.eventBindings ?? []).map((binding) => [binding.eventId, binding]),
  )
  const cooldownAnchorIds = new Set(
    (runtime?.poolCooldowns ?? [])
      .map((item) => item.anchorDecisionId)
    ...
    <div className="space-y-6">
      <section className={`${panelClass} p-6`}>
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(320px,0.7fr)] lg:items-end">
          <div>
            <h2 className="text-2xl font-black text-slate-950">会话运行态</h2>
            <p className="mt-2 text-base font-medium leading-7 text-slate-500">这里只读取已提交事实与确定性诊断，不触发适宜性判断。禁用覆盖只影响所选 Session。</p>
          </div>
          <Field label="选择会话">
            <select value={sessionId} onChange={(event) => onSessionChange(event.target.value)} className={inputClass}>
              <option value="">请选择会话</option>
              {sessions.map((session) => <option key={session.id} value={session.id}>{session.title || '未命名会话'} · {session.id}</option>)}
            </select>
          </Field>
        </div>
      </section>

      {!sessionId ? <Empty>选择一个会话后查看当前 Scene 时间、池级冷却、禁用覆盖和真实调度记录。</Empty> : !runtime ? <Empty>正在读取会话运行态…</Empty> : (
        <>
          {sessionModule && !sessionModule.effectiveEnabled ? (
            <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-amber-900">
              <AlertTriangle className="mt-0.5 shrink-0" size={20} />
              <div><p className="font-black">当前 Session 的剧情调度模块未生效</p><p className="mt-1 text-sm font-semibold leading-6">定义与覆盖仍可管理，但 turn 不会执行调度。请检查 Story 开关、Session 覆盖或系统总开关。</p></div>
            </div>
          ) : null}

          <section className="grid gap-4 lg:grid-cols-3">
            <div className={`${panelClass} p-5 lg:col-span-2`}>
              <p className="text-sm font-black text-slate-400">CURRENT SCENE TIME</p>
              {runtime.sceneTime ? <p className="mt-3 text-2xl font-black text-slate-950">{formatSceneTime(runtime.sceneTime)}</p> : (
                <div className="mt-3 flex items-start gap-3 rounded-xl bg-amber-50 p-4 text-amber-900">
                  <AlertTriangle className="mt-0.5 shrink-0" size={20} />
                  <p className="font-bold leading-6">{runtime.sceneTimeError || 'Scene 时间不可用，自动调度会安全跳过。'}</p>
                </div>
              )}
              <p className="mt-3 text-sm font-semibold text-slate-500">只有已提交的 Scene 文档净变化才为下一次非 OOC turn 留下自动调度机会；SceneTime 只负责候选窗口与冷却计算。</p>
            </div>
            <div className={`${panelClass} p-5`}>
              <p className="text-sm font-black text-slate-400">SESSION OVERRIDES</p>
              <p className="mt-3 text-3xl font-black text-slate-950">{disabledEvents.size + disabledNodes.size}</p>
              <p className="mt-2 text-sm font-semibold text-slate-500">{disabledEvents.size} 个池事件 · {disabledNodes.size} 个大纲节点</p>
            </div>
          </section>

          <section className={`${panelClass} overflow-hidden`}>
            <header className="border-b border-slate-200 px-6 py-5">
              <h3 className="text-xl font-black text-slate-950">事件池冷却诊断</h3>
              <p className="mt-1 text-sm font-semibold text-slate-500">只使用最近一次已提交的自动 pool lane 成功注入作为锚点；手动、大纲、延期与错误判断不改变池级冷却。</p>
            </header>
            {!runtime.poolCooldowns.length ? <div className="px-6 py-10 text-center text-sm font-semibold text-slate-400">当前 Story 没有事件池</div> : (
              <div className="grid gap-4 p-6 lg:grid-cols-2">
                {runtime.poolCooldowns.map((cooldown) => {
                  const pool = poolById.get(cooldown.poolId)
                  const statusLabel = cooldown.status === 'cooling_down'
                    ? `冷却剩余 ${formatMinutes(cooldown.remainingMinutes)}`
                    : cooldown.status === 'scene_time_unavailable'
                      ? 'SceneTime 不可用'
                      : cooldown.status === 'inactive'
                        ? '未启用池级冷却'
                        : '可参与候选'
                  const statusTone = cooldown.status === 'cooling_down'
                    ? 'rose'
                    : cooldown.status === 'scene_time_unavailable'
                      ? 'amber'
                      : 'emerald'
                  return (
                    <article key={cooldown.poolId} className={`rounded-2xl border p-5 ${cooldown.blocksAutomaticSelection ? 'border-rose-200 bg-rose-50/60' : 'border-slate-200 bg-slate-50'}`}>
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div><p className="text-lg font-black text-slate-950">{pool?.name ?? `事件池 #${cooldown.poolId}`}</p><p className="mt-1 text-xs font-bold text-slate-400">#{cooldown.poolId} · 配置 {formatMinutes(cooldown.cooldownMinutes)}</p></div>
                        <Tag tone={statusTone}>{statusLabel}</Tag>
                      </div>
                      <p className="mt-4 text-sm font-semibold leading-6 text-slate-600">{cooldown.reason}</p>
                      <dl className="mt-4 grid grid-cols-2 gap-3 text-xs">
                        <div><dt className="font-black text-slate-400">已过时间</dt><dd className="mt-1 font-bold text-slate-700">{formatMinutes(cooldown.elapsedMinutes)}</dd></div>
                        <div><dt className="font-black text-slate-400">锚点</dt><dd className="mt-1 font-bold text-slate-700">{cooldown.anchorDecisionId ? `Decision #${cooldown.anchorDecisionId} · Turn ${cooldown.anchorTurnId}` : '无'}</dd></div>
                        <div><dt className="font-black text-slate-400">锚点事件</dt><dd className="mt-1 font-bold text-slate-700">{cooldown.anchorEventId ? eventById.get(cooldown.anchorEventId)?.title ?? `#${cooldown.anchorEventId}` : '无'}</dd></div>
                        <div><dt className="font-black text-slate-400">锚点时间</dt><dd className="mt-1 font-bold text-slate-700">{cooldown.anchorSceneTime ? formatSceneTime(cooldown.anchorSceneTime) : '无'}</dd></div>
                      </dl>
                    </article>
                  )
                })}
          ...
          <section className={`${panelClass} overflow-hidden`}>
            <header className="border-b border-slate-200 px-6 py-5">
              <h3 className="text-xl font-black text-slate-950">Session 禁用覆盖</h3>
              <p className="mt-1 text-sm font-semibold text-slate-500">事件覆盖只影响自动 pool lane；大纲节点使用独立覆盖。大纲绑定事件本身也不会进入 pool lane。</p>
            </header>
            <div className="grid divide-y divide-slate-100 xl:grid-cols-2 xl:divide-x xl:divide-y-0">
              <div className="p-6">
                <h4 className="mb-4 flex items-center gap-2 font-black text-slate-800"><Dices size={18} className="text-violet-500" />事件池</h4>
                <div className="space-y-2">
                  {schedule.events.map((event) => {
                    const binding = bindingByEventId.get(event.id)
                    return (
                      <div key={event.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3">
                        <div className="min-w-0"><p className="truncate text-sm font-black text-slate-800">{event.title}</p><p className="mt-0.5 text-xs font-semibold text-slate-400">#{event.id}{binding?.outlineBound ? ` · 大纲专用（${binding.outlineNodeReferenceCount} 个节点）` : ' · 池内候选'}</p></div>
                        <Toggle disabled={busy} checked={!disabledEvents.has(event.id)} label={disabledEvents.has(event.id) ? '池内已禁用' : '池内启用'} onChange={(checked) => onEventOverride(event.id, !checked)} />
                      </div>
                    )
                  })}
                </div>
              ...
              <div className="p-6">
                <h4 className="mb-4 flex items-center gap-2 font-black text-slate-800"><GitBranch size={18} className="text-violet-500" />大纲节点</h4>
                <div className="space-y-2">
                  {schedule.outlines.flatMap((outline) => outline.nodes.map((node) => (
                    <div key={node.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3">
                      <div className="min-w-0"><p className="truncate text-sm font-black text-slate-800">{outline.name} · {eventById.get(node.eventId)?.title ?? `事件 #${node.eventId}`}</p><p className="mt-0.5 text-xs font-semibold text-slate-400">{formatSceneTime(node.scheduledTime)}</p></div>
                      <Toggle disabled={busy} checked={!disabledNodes.has(node.id)} label={disabledNodes.has(node.id) ? '已禁用' : '启用'} onChange={(checked) => onNodeOverride(node.id, !checked)} />
                    </div>
                  )))}
                </div>
              </div>
          ...
          <section className={`${panelClass} overflow-hidden`}>
            <header className="flex flex-col gap-3 border-b border-slate-200 px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-3"><History size={21} className="text-violet-500" /><div><h3 className="text-xl font-black text-slate-950">调度判断记录</h3><p className="mt-1 text-sm font-semibold text-slate-500">卡片摘要用于浏览；展开后可检查完整持久字段与原始事件快照。</p></div></div>
              {viewingOlder ? <button type="button" onClick={onLatest} className={quietButton}>回到最新</button> : null}
            </header>
            <div className="divide-y divide-slate-100">
              {!runtime.decisions.length ? <div className="px-6 py-12 text-center text-sm font-semibold text-slate-400">尚无已提交的调度判断</div> : runtime.decisions.map((decision) => {
                const snapshotTitle = typeof decision.eventSnapshot.eventTitle === 'string' ? decision.eventSnapshot.eventTitle : `事件 #${decision.eventId}`
                const tone = decision.decisionStatus === 'triggered' ? 'emerald' : decision.decisionStatus === 'deferred' ? 'amber' : 'rose'
                const facts = [
                  ['Decision ID', String(decision.id)],
                  ['Session ID', decision.sessionId],
                  ['Turn ID', String(decision.turnId)],
                  ['Source', `${decision.sourceKind} / ${decision.sourceId}`],
                  ['Event ID', String(decision.eventId)],
                  ['Container ID', String(decision.containerId)],
                  ['Decision status', decision.decisionStatus],
                  ['Dispatch mode', decision.dispatchMode],
                  ['Selection origin', decision.selectionOrigin],
                  ['SceneTime ordinal', decision.sceneTimeOrdinal === null ? 'null' : String(decision.sceneTimeOrdinal)],
                  ['Reason', decision.reason || ''],
                  ['Error code', decision.errorCode || ''],
                  ['Error message', decision.errorMessage || ''],
                  ['Version', String(decision.version)],
                  ['Created at', decision.createdAt],
                  ['Updated at', decision.updatedAt],
                ]
                return (
                  <article key={decision.id} className="px-6 py-5">
                    <div className="grid gap-3 lg:grid-cols-[140px_minmax(0,1fr)_auto] lg:items-start">
                      <div><p className="text-sm font-black text-slate-800">Turn {decision.turnId}</p><p className="mt-1 text-xs font-semibold text-slate-400">{decision.sceneTime ? formatSceneTime(decision.sceneTime) : '无 SceneTime'}</p></div>
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <h4 className="font-black text-slate-900">{snapshotTitle}</h4>
                          <Tag tone={tone}>{decision.decisionStatus === 'triggered' ? '已触发' : decision.decisionStatus === 'deferred' ? '已延期' : '判断错误'}</Tag>
                          <Tag>{decision.sourceKind === 'outline' ? '大纲' : '事件池'}</Tag>
                          {decision.selectionOrigin === 'manual' ? <Tag tone="violet">手动</Tag> : null}
                          {cooldownAnchorIds.has(decision.id) ? <Tag tone="amber">当前池冷却锚点</Tag> : null}
                        </div>
                        <p className="mt-2 text-sm font-medium leading-6 text-slate-600">{decision.reason || decision.errorMessage || '无补充说明'}</p>
                        {decision.errorCode ? <p className="mt-1 text-xs font-bold text-rose-500">{decision.errorCode}</p> : null}
                      </div>
                      <Tag tone={decision.dispatchMode === 'forced' ? 'rose' : 'violet'}>{decision.dispatchMode === 'forced' ? '强制' : '软约束'}</Tag>
                    </div>
                    <details className="mt-4 rounded-xl border border-slate-200 bg-slate-50">
                      <summary className="cursor-pointer px-4 py-3 text-sm font-black text-violet-700">展开完整决策信息</summary>
                      <div className="border-t border-slate-200 p-4">
                        <dl className="grid gap-x-6 gap-y-3 sm:grid-cols-2 xl:grid-cols-3">
                          {facts.map(([label, value]) => <div key={label}><dt className="text-xs font-black uppercase tracking-wide text-slate-400">{label}</dt><dd className="mt-1 break-words text-sm font-bold text-slate-700">{value || '—'}</dd></div>)}
                          <div><dt className="text-xs font-black uppercase tracking-wide text-slate-400">SceneTime</dt><dd className="mt-1 text-sm font-bold text-slate-700">{decision.sceneTime ? formatSceneTime(decision.sceneTime) : 'null'}</dd></div>
                        </dl>
                        <div className="mt-5"><p className="text-xs font-black uppercase tracking-wide text-slate-400">Event snapshot</p><pre className="mt-2 max-h-96 overflow-auto rounded-xl bg-slate-950 p-4 text-xs leading-6 text-slate-100">{JSON.stringify(decision.eventSnapshot, null, 2)}</pre></div>
                      </div>
                    </details>
                  </article>
                )
            ...
            {runtime.nextBeforeId ? <footer className="border-t border-slate-200 px-6 py-4 text-center"><button type="button" onClick={() => onEarlier(runtime.nextBeforeId!)} className={quietButton}>查看更早记录</button></footer> : null}
          </section>
        </>
      )}
    </div>
...
function PlotSchedulingContent() {
  ...
  return (
    <div className="min-w-0 px-5 py-7 lg:px-8 2xl:px-10 2xl:py-9">
      <header className="mb-7 flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between"><div className="flex items-start gap-4"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-violet-100 text-violet-700"><Route size={28} /></span><div><p className="text-sm font-black uppercase tracking-[0.2em] text-violet-500">Dynamic plot orchestration</p><h1 className="mt-1 text-3xl font-black text-slate-950 2xl:text-4xl">剧情动态调度</h1><p className="mt-2 max-w-4xl text-base font-medium leading-7 text-slate-500">以 Story 为边界组织线性大纲与事件池；Scene 文档发生已提交净变化后，下一次非 OOC turn 才进行一次自动候选选择。</p></div></div><div className="w-full xl:w-[420px]"><Field label="当前故事"><select value={storyId ?? ''} onChange={(event) => setStoryId(Number(event.target.value) || null)} className={inputClass}><option value="">请选择故事</option>{stories.map((story) => <option key={story.id} value={story.id}>{story.title}</option>)}</select></Field></div></header>
