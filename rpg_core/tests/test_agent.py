...
async def test_ensure_initialized_is_idempotent(monkeypatch):
    ...
    monkeypatch.setattr(agent_module, "OpenAIProvider", MagicMock(return_value=object()))
    ...
    monkeypatch.setattr(agent_module, "_resolve_sub_agent_provider", lambda **kwargs: (None, SimpleNamespace(mode="shared")))
    monkeypatch.setattr(
        agent_module,
        "settings",
        SimpleNamespace(
