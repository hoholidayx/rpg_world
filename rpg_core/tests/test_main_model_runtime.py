...
class _SelectionService:
    ...
    async def resolve_session(self, session_id: str):  # noqa: ANN201
...
async def test_main_model_runtime_reuses_provider_for_same_effective_key(monkeypatch) -> None:
    selections = _SelectionService()
    selections.selections["s1"] = _selection("chat-a", "configured-a")
    calls: list[str] = []

    class _Manager:
        async def get_provider(self, _biz_key, *, provider_key):  # noqa: ANN001, ANN201
    ...
    first = await runtime.provider_for("s1")
    selections.selections["s1"] = _selection("chat-a", "updated-config")
    second = await runtime.provider_for("s1")
...
async def test_main_model_runtime_switches_provider_only_for_new_key(monkeypatch) -> None:
    selections = _SelectionService()
    ...
    assert (await runtime.provider_for("s1")).get_default_model() == "chat-a"
    selections.selections["s1"] = _selection("chat-b", "b")
    assert (await runtime.provider_for("s1")).get_default_model() == "chat-b"
    assert calls == ["chat-a", "chat-b"]


async def test_main_model_runtime_requires_catalog_selection() -> None:
    runtime = MainModelRuntime(
        selection_service=_SelectionService(),
    )

    try:
        await runtime.resolve("missing")
