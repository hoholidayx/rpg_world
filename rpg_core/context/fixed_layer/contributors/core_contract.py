...
STATE_SYNC_BEFORE_NARRATION_RULE = (
    "先在内部确定本轮叙事后果和最终追踪值，只核验与本轮输入及后果相关的 scene/状态字段。"
    "动态段明确已由状态预处理暂存的变化，直接以该工作值为准且不得重复写。"
    "其余凡实际、持久、已经确定且尚未同步的变化，必须在输出任何 RP 正文前调用对应状态工具；"
    "工具调用轮不得夹带 RP 正文，"
    ...
    def _core_rp_contract(self) -> FixedLayerSection:
        return FixedLayerSection(
            ...
            content=(
                ...
                "必须以该绑定为准。\n"
                "- [scene] 标签块和普通状态表是当前 turn 的权威运行时工作快照；"
                "其相对本轮输入的阶段由对应动态段明确。不得无依据覆盖，"
                "但本轮剧情已经确定的未同步变化必须同步。\n"
                "- 默认保留玩家能动性：不要替玩家角色说话、行动、描写心理或作出选择；"
                "只有当前 turn 的后置 Message Mode RP Module 明确声明 GM 托管时例外。\n"
                "- 你可以描写环境、后果、NPC 行动、风险与线索。\n"
