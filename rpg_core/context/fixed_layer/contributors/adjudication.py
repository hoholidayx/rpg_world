...
class AdjudicationAuthorityFixedLayerContributor(FixedLayerContributor):
    ...
    def __init__(self, *, include_lookup_guidance: bool = False) -> None:
        if not isinstance(include_lookup_guidance, bool):
            raise TypeError("include_lookup_guidance must be a boolean")
        self._include_lookup_guidance = include_lookup_guidance
    ...
    def get_fixed_contribution(self) -> FixedLayerContribution:
        evidence_guidance = (
            ...
            if self._include_lookup_guidance
            else (
                "- 只能依据当前阶段实际提供的事实进行判断；证据不足时不得"
                "凭空补足，也不得假定存在未提供的查询能力。\n"
            )
        )
