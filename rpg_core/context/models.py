"""Structured data models for the RPG context pipeline."""
...
class Message:
    ...
    def to_provider_dict(self) -> dict[str, object]:
        data: dict[str, object] = {
            MsgKey.ROLE: self._role.value,
            MsgKey.CONTENT: self._content,
        }
    ...
    def to_persistence_dict(self) -> dict[str, object]:
        ...
        if self._turn_id:
            data[MsgKey.TURN_ID] = self._turn_id
        if self._seq_in_turn:
            data[MsgKey.SEQ_IN_TURN] = self._seq_in_turn
        return data
