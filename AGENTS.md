...
- Plot Scheduler 是 Story 级 RP Module：多个线性大纲与多个 `random | sequential` 事件池可同时存在，每个 IC/GM turn 最多选择一个到期大纲节点和一个池事件，同一事件不得在同轮被两个 lane 重复注入，OOC 不调度。Scene 时间统一使用 `SceneTime` 严格解析当前 scene 的“时间”字段；强制候选到时直接注入，软候选通过 `agent.plot_scheduler` 业务路由读取完整 fixed layer、当前 scene/状态表、最近 N 个完整原始 IC/GM turn 与当前输入判断适宜性。软判断 `reason` 必须受代码长度上限约束；Context 门禁按最长两条指令、事件/容器名称和有界元数据保守预留，不得依赖模型自行保持简短。`deferred | error` 不阻断主 turn，并跳过配置数量的完整 IC/GM turn 后重试。调度注入只进入 RP Module 动态层，不写 SSE/历史正文；决策与消息、Narrative Outcome、scene/status 原子提交，失败或取消随 scratch 丢弃。
- Plot 定义挂在 Story；Session 只保存池事件和大纲节点的禁用覆盖。事件禁用只影响池 lane，节点禁用只影响大纲 lane；大纲节点不可重复，池事件重复使用世界内分钟冷却。池 lane 的触发/延期/冷却 identity 是稳定 `event_id`，把事件移动到其它池不得重置其不重复语义或重试状态，`container_id` 只保留当时来源快照。`/clear` 清决策账本但保留覆盖；派生只复制分支点前 `triggered` 与覆盖，不复制 `deferred/error`。迁移新增默认 RP Module 时不追溯挂载既有 Story。
- Plot WebUI 固定使用独立 `/plot-scheduling` 页面管理 Story 定义、Session 覆盖和只读运行态，不往 SessionRoom 增加 HUD，也不轮询或触发判断 LLM。决策历史按稳定自增 `id DESC` 排序并使用 `beforeId` 游标；公开页大小最大 200，服务层允许额外一条内部 lookahead，不能改回按 `turn_id` 游标而造成同 turn 两条 lane 记录漏页。
...
- turn hooks 只允许固定类型化阶段：`StatusPreflightHook → PlotSchedulingPreflightHook → MemoryRecallHook → runner/commit → PostCommitHooks`。Status preflight 未处理异常终止并 discard；Plot 强制项不调用 LLM，软判断失败只暂存 error 并继续；memory recall 失败 warning-and-continue；story-memory/summary post-commit 逐项隔离且不回滚。不要增加事件总线、动态优先级、运行时重排或第三方 hook 注册。
- Agent 普通正文在命令分发和角色校验后先解析 RP Module 与 Plot Schedule 不可变快照，再在创建 `AgentTurnTransaction` 前执行主 Context 窗口门禁；门禁只估算当前 Context，不计本次 input，并为同轮最多两条 Plot directive 保守预留 token，达到 `settings.context_window_reject_threshold_ratio` 时拒绝正文但始终允许斜杠命令。通过门禁后，`send/send_stream` 的 user/assistant message、Narrative Outcome、Plot decision 与 scene/status document 才进入内存 scratch，LLM 完整成功后在短 commit 点统一写 main history、backup history、剧情裁定、调度账本和状态表；summary compression 和 story memory extraction 仍只作为 commit 后副作用运行，失败只记录 warning。
