...
- StatusSubAgent 固定执行 Outcome 判定 → 状态目标路由 → scene/逐表即时更新；Outcome 只判断当前输入，不得重新裁定近期历史旧悬念。确定性 eligibility 未开放时不得注册 Outcome 工具或发起 Outcome Provider 调用，直接进入状态 Route。表目标包含 `keys + structure`，纯创建允许空 keys，结构目标获得完整表快照和结构工具，value-only 目标只获得所选 rows。每次 LLM 调用只获得一个目标、table/key 双重 allowlist 和实际可用 schema。更新按目标使用内存 checkpoint，单目标失败只回滚该目标并继续；checkpoint 创建/恢复失败终止并 discard 整个 turn。retry/edit/truncate 重抽裁定，但不回滚已提交状态表。
