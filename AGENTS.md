...
- Narrative Outcome 只向 LLM 暴露 `rp_story_outcome(reason, actor?)`；待裁定契约与最终结果都只进入 Hot History 后的 `RP_MODULES` 动态层，永不进入 Fixed Layer。Outcome eligibility 使用同一个确定性 turn 门禁同时控制 Status preflight、动态 section 与主 Agent registry/schema：OOC 始终关闭；GM 默认关闭，只有明确随机/检定/运气或明确保留未知外部反应时开放；任意模式的已确认事实、固定推进和事实更正默认关闭，强随机/明确未知表达优先。每 turn 最多一个五级结果；第一次成功裁定后，同步与流式主 runner 必须在下一 Provider round 重新投影动态 section 和 schema，并立即从执行 allowlist 移除该工具，同一响应批次中的后续重复调用也必须拒绝；底层幂等复用只作为内部最后防线。不得向 LLM 暴露 Dice 表达式、DC、权重或随机数；Dice 仅保留手动 `/roll`、`/check_dc` 与解析调试。权重五项均为 `0..100` 且总和严格为 100，`success_with_cost` 必须完整达成原目标。Outcome code/sample/来源一致性由 `NarrativeOutcomeLedgerService` 校验，`NarrativeOutcomeDataService` 只追加调用方准备的 typed ledger row。
...
- `agent.status_sub_agent.preflight_state_updates` 在 turn policy 中冻结状态写入 owner：字段缺失时为兼容旧配置回退 `true`，当前 base 配置显式使用 `false`。`true` 时 StatusSubAgent 固定执行 Outcome 判定 → 状态目标路由 → scene/逐表即时更新；`false` 时仍保留符合 eligibility 的 Outcome 预裁定，但不向 StatusSubAgent 注册 scene/普通状态工具、不运行 Route/Update，由主 Agent 用现有状态工具直接写 scratch，派生 Session bootstrap 不受影响。Outcome 只判断当前输入，不得重新裁定近期历史旧悬念。确定性 eligibility 未开放时不得注册 Outcome 工具或发起 Outcome Provider 调用；状态预写开启时直接进入状态 Route，关闭且无 Outcome 工具时不调用 Status Provider。表目标包含 `keys + structure`，纯创建允许空 keys，结构目标获得完整表快照和结构工具，value-only 目标只获得所选 rows。每次 LLM 调用只获得一个目标、table/key 双重 allowlist 和实际可用 schema。更新按目标使用内存 checkpoint，单目标失败只回滚该目标并继续；checkpoint 创建/恢复失败终止并 discard 整个 turn。主 Context 的 Scene 与普通状态表必须区分 turn 起始值和前置已暂存工作值，持久 user message 只保存 pre-turn Scene 数据快照；已暂存值不得重复写。Plot hook 顺序和规则不参与 A/B，关闭状态预写时仍在主 Agent 前按 turn 起始 Scene 消费上一轮机会。retry/edit/truncate 重抽裁定，但不回滚已提交状态表。
