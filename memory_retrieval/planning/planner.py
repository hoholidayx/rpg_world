"""Business-neutral query planning for text and vector retrieval."""
...
class BaseQueryPlanner(ABC):
    ...
    async def plan_context(self, context: RetrievalQuery) -> QueryPlan:
        return _augment_with_context(await self.plan(context.text), context)
...
class RuleBasedQueryPlanner(BaseQueryPlanner):
    ...
    async def plan_context(self, context: RetrievalQuery) -> QueryPlan:
        return _augment_with_context(self.plan_sync(context.text), context)
...
def _augment_with_context(plan: QueryPlan, context: RetrievalQuery) -> QueryPlan:
    expansions = _dedupe([
        *plan.expanded_queries,
        *context.expansions,
    ])[:_MAX_QUERY_VARIANTS]
    keyword_context = [value for value in context.keyword_context if value]
