"""Loop-owned coordinator for watched retrieval sources and indexes."""
...
from memory_retrieval.chunker import Chunker, FileTextExtractor
...
class SourceWatcher(Protocol):
    """Minimal watcher contract injected by the application composition root."""

    def register(self, path: Path, callback: Callable[[], None]) -> None: ...

    def unregister(self, path: Path, callback: Callable[[], None]) -> None: ...
...
class VectorIndexManager:
    ...
    def _register(self, source: WatchSource) -> None:
        ...
        self._watcher.register(resolved_target, callback)
