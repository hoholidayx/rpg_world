"""Assembled retriever combining sqlvec, keyword, and raw markdown search."""
...
from memory_retrieval.retrieval.policy import (
    AcceptAllCandidatePolicy,
    CandidatePolicy,
)
...
from memory_retrieval.retrieval.scoring import apply_hybrid_scores, exact_and_fuzzy_scores
...
class HybridRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        self._candidate_policy = candidate_policy or AcceptAllCandidatePolicy()
    ...
    async def _search_plan_candidates(self, plan: QueryPlan, top_k: int) -> list[MemoryCandidate]:
        ...
        candidates = [
            ...
            if self._candidate_policy.is_eligible(candidate.metadata)
        ]
        from loguru import logger
    ...
    async def _finalize(
        ...
        for candidate in candidates:
            ...
            granularity, score = self._candidate_policy.granularity(
                candidate.metadata
            )
...
def _candidate_count(
    ...
    *groups: list[MemoryCandidate],
    ...
    return len({
        ...
        if policy.is_eligible(candidate.metadata)
    })
