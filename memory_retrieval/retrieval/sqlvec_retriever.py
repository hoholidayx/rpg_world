"""SQL-backed vector retriever for generic recall."""
...
from memory_retrieval.retrieval.retriever import BaseRetriever, _similarity
...
class SqlVecRetriever(BaseRetriever):
    ...
    async def search(self, query: str, top_k: int = 5) -> list[MemoryCandidate]:
        ...
        for record, distance in raw:
            if not self._candidate_policy.is_eligible(record.metadata):
                continue
            candidates.append(
                MemoryCandidate(
                    memory_id=record.id,
