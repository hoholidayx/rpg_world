"""Async-first RP recall facade with one coordinator per Agent session."""
...
if TYPE_CHECKING:
    ...
    from memory_retrieval.planning.planner import BaseQueryPlanner
    from memory_retrieval.rerank.base import MemoryReranker
    from memory_retrieval.retrieval.retriever import BaseRetriever
    from memory_retrieval.storage.vector_store import VectorStore
    from memory_retrieval.vector_index_manager import VectorIndexManager
    from rpg_memory.recall.config import MemoryRecallConfig
...
class MemoryRecallManager:
    ...
    def create(
        ...
        mem_cfg: "MemoryRecallConfig",
        *,
        watcher: object | None = None,
    ) -> "MemoryRecallManager | None":
        """Build a local shell without contacting LLM Service."""
        if not mem_cfg.enabled:
            logger.info("[MemoryRecallManager] disabled by config")
            return None
        ...
        logger.info(
            "[MemoryRecallManager] local shell ready — session_dir={} db_path={} hybrid={} top_k={}",
            session_dir,
            get_vector_db_path,
    ...
    def __init__(
        ...
        *,
        ...
        mem_cfg: "MemoryRecallConfig | None" = None,
        ...
        self._planner_ready = not bool(
            mem_cfg and mem_cfg.query_planner_enabled
        )
        self._reranker_ready = not bool(
            mem_cfg and mem_cfg.rerank_enabled
        )
    ...
    async def initialize(self) -> None:
        ...
        async with self._init_lock:
            ...
            if self._store is not None and self._mem_cfg is not None:
                ...
                if changed:
                    logger.warning(
                        "[MemoryRecallManager] content index fingerprint changed; derived index reset"
                    )
            if (
            ...
            logger.info("[MemoryRecallManager] initialized in text-index-first mode")
    ...
    async def recall(self, query: str | RecallQueryContext) -> list[RecallItem]:
        ...
        async with self._operation_lock:
            ...
            logger.info(
                "[MemoryRecallManager] recall start — query={!r} top_k={}",
        ...
        logger.info(
            "[MemoryRecallManager] recall done — query={!r} items={}",
            context.current_input,
            len(items),
        )
        for index, item in enumerate(items):
            for line in format_recall_item(index, item).splitlines():
                logger.info("[MemoryRecallManager] {}", line)
        return items
    ...
    async def reindex(self) -> None:
        ...
        if self._index_manager is None:
            logger.warning("[MemoryRecallManager] reindex skipped: index manager unavailable")
            return
        ...
        async with self._remote_lock:
            ...
            async def resolve(label: str, biz_key: str):
                try:
                    ...
                    logger.warning(
                        "[MemoryRecallManager] {} provider unavailable; keep fallback: {}",
                        label,
                        exc,
                    )
                    return label, None
            ...
            if embedding is not None:
                try:
                    ...
                    logger.warning(
                        "[MemoryRecallManager] embedding dimension unavailable; keep text fallback: {}",
                        exc,
                    )
    ...
    def _build_sources(session_dir: str):
        from memory_retrieval.vector_index_manager import WatchSource
    ...
    def _build_raw_md_search(sources, rule_based_planner=None):
        from memory_retrieval.retrieval.raw_md_grep_search import RawMarkdownGrepSearch
    ...
    def _build_store(
        ...
        try:
            ...
            logger.warning("[MemoryRecallManager] store init failed: {}", exc)
            return None

    @staticmethod
    def _build_chunker(mem_cfg: "MemoryRecallConfig"):
        from memory_retrieval.chunker import Chunker
    ...
    def _build_sqlvec_retriever(store, embedding):
        if store is None or embedding is None or not store.vector_enabled:
            return None
        from memory_retrieval.retrieval.sqlvec_retriever import SqlVecRetriever
        from rpg_memory.recall.policy import RPRecallCandidatePolicy

        return SqlVecRetriever(
            store=store,
            embedding=embedding,
            candidate_policy=RPRecallCandidatePolicy(),
        )
    ...
    def _build_keyword_retriever(store, keyword_limit: int):
        if store is None:
            return None
        from memory_retrieval.retrieval.keyword_retriever import KeywordRetriever
    ...
    def _build_raw_md_retriever(fallback_search):
        if fallback_search is None:
            return None
        from memory_retrieval.retrieval.raw_md_retriever import RawMarkdownRetriever
    ...
    def _build_retriever(
        ...
        *,
        ...
        from memory_retrieval.retrieval.hybrid_retriever import HybridRetriever
        from rpg_memory.recall.policy import RPRecallCandidatePolicy
    ...
    @staticmethod
    def _build_rule_based_query_planner(mem_cfg: "MemoryRecallConfig"):
        from memory_retrieval.planning.planner import RuleBasedQueryPlanner

        return RuleBasedQueryPlanner(
            jieba_dict=mem_cfg.jieba_dict or None
        )

    @classmethod
    def _build_query_planner_with_provider(cls, mem_cfg, provider):
        from rpg_memory.recall.planner import OpenAIQueryPlanner
        from memory_retrieval.planning.planner import FallbackQueryPlanner
    ...
    def _build_reranker_with_provider(mem_cfg, provider):
        ...
        from rpg_memory.recall.rerank import build_rp_pointwise_prompt

        return PointwiseMemoryReranker(
            provider,
            rerank_weight=mem_cfg.rerank_score_weight,
            provider_label="llm_service",
            prompt_builder=build_rp_pointwise_prompt,
        )
    ...
    def _items_from_candidates(candidates) -> list[RecallItem]:
        ...
        return MemoryRecallManager._items(raw)


def _content_index_fingerprint(mem_cfg: "MemoryRecallConfig") -> str:
    payload = {
        "schema_version": 3,
...
def _log_query_plan(stage: str, plan) -> None:  # noqa: ANN001
    logger.info(
        "[MemoryRecallManager] {} plan — planner={} type={} normalized={!r} keyword_queries={} raw_md_terms={} expanded_queries={}",
        stage,
        plan.planner_source,
        plan.query_type,
