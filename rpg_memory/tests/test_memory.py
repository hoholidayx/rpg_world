"""Retrieval engine and RP recall integration tests."""
...
from rpg_memory.recall.manager import MemoryRecallManager, RecallItem
from rpg_memory.recall.query import RecallQueryContext
from memory_retrieval.storage.types import ChunkRecord, IndexedFileState
from memory_retrieval.storage.vector_store import VectorStore
from memory_retrieval.storage import vector_store as vector_store_module
from memory_retrieval.keyword_tokenizer import (
...
from memory_retrieval.retrieval.hybrid_retriever import _build_rerank_query
from memory_retrieval.retrieval.raw_md_grep_search import RawMarkdownGrepSearch, _frontmatter_metadata
...
from memory_retrieval.rerank import (
...
from rpg_memory.recall.policy import (
    RPRecallCandidatePolicy,
    granularity_score,
    resolve_memory_granularity,
)
from rpg_memory.recall.rerank import build_rp_pointwise_prompt
from rpg_memory.tests.conftest import (
    FakeEmbedding,
    FakeFallbackSearch,
    FakeRetriever,
    FakeStore,
...
async def test_rule_planner_augments_query_with_rp_turn_context():
    ...
    plan = await planner.plan_context(context.to_retrieval_query())
...
async def test_sqlvec_searches_current_and_context_expansions():
    ...
    retriever = SqlVecRetriever(  # type: ignore[arg-type]
        store,
        embedding,
        candidate_policy=RPRecallCandidatePolicy(),
    )
...
def test_memory_manager_create_disabled(fake_recalled_store):
    ...
    manager = MemoryRecallManager.create(
...
def test_memory_manager_reranker_uses_settings_rerank_score_weight():
    ...
    reranker = MemoryRecallManager._build_reranker_with_provider(
        SimpleNamespace(
    ...
    assert reranker._score_provider._prompt_builder is build_rp_pointwise_prompt
...
def test_pointwise_rerank_prompt_is_short_and_strict():
    ...
    assert "评分：" in prompt
    assert "候选：" in prompt
    assert "RP：" not in prompt
    assert len(prompt) < 180


def test_rp_pointwise_rerank_prompt_preserves_epistemic_semantics():
    prompt = build_rp_pointwise_prompt(
        "船长是否已经完成承诺？",
        MemoryCandidate(
            memory_id=1,
            content="有人传闻船长计划回来。",
            metadata={"memory_kind": "event", "epistemic_status": "rumor"},
        ),
    )

    assert "传闻/推测/不确定≠事实" in prompt
    assert "承诺/计划≠完成" in prompt
    assert "类型：event；认知：rumor" in prompt
...
def test_raw_md_expanded_terms_fallback_planner_uses_jieba_dict(monkeypatch):
    ...
    monkeypatch.setattr(
        "memory_retrieval.retrieval.raw_md_grep_search.RuleBasedQueryPlanner",
        CapturingPlanner,
    )
...
async def test_memory_manager_recall_logs_raw_md_plan_terms(monkeypatch, fake_recalled_store):
    ...
    class FakePlanner:
        ...
        async def plan_context(self, context):  # noqa: ANN001
            return await self.plan(context.text)

    monkeypatch.setattr(
        "rpg_memory.recall.manager.logger.info",
        capture,
    )
    manager = MemoryRecallManager(
        recalled_store=fake_recalled_store,
...
async def test_memory_manager_recall_without_retriever(fake_recalled_store):
    manager = MemoryRecallManager(recalled_store=fake_recalled_store, retriever=None)
...
async def test_hybrid_retriever_planner_failure_returns_empty(monkeypatch):
    ...
    monkeypatch.setattr(
        "memory_retrieval.retrieval.hybrid_retriever.RuleBasedQueryPlanner.plan",
        boom,
    )
