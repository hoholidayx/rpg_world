...
uv run python -m rpg_memory.benchmark suite

# 成功后生成独立的 Git 跟踪中文总结并更新索引
uv run python -m rpg_memory.benchmark suite --record

# 不访问 LLM Service；数据缓存缺失时仍可能下载所选数据集
uv run python -m rpg_memory.benchmark suite --offline-only
...
# 只准备指定数据集；有效缓存不会联网
uv run python -m rpg_memory.benchmark prepare \
