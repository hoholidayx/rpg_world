"""RP recall 调试入口，用于手动验证向量记忆系统的各项功能。

用法::

    uv run python -m channels.cli.memory_recall
    uv run python -m channels.cli.memory_recall --session mygame
    uv run python -m channels.cli.memory_recall --query "森林"
"""
...
from rpg_memory.recall.manager import MemoryRecallManager, format_recall_item
from rpg_core.utils.watcher import get_watcher
from rpg_data.services import get_data_service_gateway

if TYPE_CHECKING:
    from rpg_memory.recall.manager import RecallItem
...
def create_manager(workspace: str, session: str) -> MemoryRecallManager | None:
    """2. 创建不访问 LLM Service 的 MemoryRecallManager 本地壳。"""
    _print_separator("2. MemoryRecallManager.create() 创建本地壳")

    recalled = RecalledMemoryStore()
    session_root = _session_root(session)
    mm = MemoryRecallManager.create(
    ...
    if mm is None:
        print("  ❌ MemoryRecallManager.create() 返回 None")
        return None

    print("  ✅ MemoryRecallManager 创建成功")
...
async def initialize_manager(mm: MemoryRecallManager, session: str) -> None:
...
async def preview_recall(mm: MemoryRecallManager, query: str) -> None:
    """4. 执行召回测试。"""
...
async def _loop(mm: MemoryRecallManager, workspace: str, session: str) -> None:
    ...
    commands = {
        ...
        "info": "info               — MemoryRecallManager 状态",
        "help": "help               — 显示帮助",
        "quit": "quit               — 退出",
    }
    ...
    while True:
        ...
        if cmd == "quit":
            ...
            _print_separator("MemoryRecallManager 状态")
...
async def main_async() -> None:
    ...
    parser = argparse.ArgumentParser(description="MemoryRecallManager CLI 测试")
    ...
    mm: MemoryRecallManager | None = None

    print(f"\n🔧 MemoryRecallManager 测试 — workspace={workspace!r} session={args.session!r}\n")
