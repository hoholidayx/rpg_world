...
class TestAgentManager:
    ...
    def test_cache_key(self):
        key1 = AgentManager._cache_key("default", None)
        key2 = AgentManager._cache_key("default", "sk-xxx")
        key3 = AgentManager._cache_key("mygame", None)
        assert key1 == "default::"
        assert key2 == "default::sk-xxx"
        assert key3 == "mygame::"
    ...
    def test_get_or_create_twice_returns_same(self):
        a1 = AgentManager.get_or_create(session_id="test1", api_key="key1")
        a2 = AgentManager.get_or_create(session_id="test1", api_key="key1")
        assert a1 is a2  # 同一实例

    def test_get_or_create_different_keys(self):
        a1 = AgentManager.get_or_create(session_id="test", api_key="key1")
        a2 = AgentManager.get_or_create(session_id="test", api_key="key2")
    ...
    def test_ensure_initialized_once(self):
        with patch(_AGENT_PATCH_PATH, return_value=None):
            ...
            import asyncio
            asyncio.run(AgentManager.ensure_initialized())
